...
function parseTMX(xmlText){
  ...
  return { width, height, layers, doors, enemySpawns, crateSpawns, barrelSpawns, platformSpawns, laserSpawns, buttonSpawns, liquidSpawns };
}
...
function updatePlayer(dt){
  ...
  if(liquidDef && liquidDef.damageTick > 0 && player.invulnT <= 0 && !gameOver){
    player.hp = Math.max(0, player.hp - liquidDef.damageTick);
    player.invulnT = LIQUID_LAVA_TICK_INVULN;
    if(player.hp <= 0) gameOver = true;
  }
  ...
  const currentMaxSpeed = (player.crouching ? PHYS.moveSpeed * CROUCH_SPEED_MULT : PHYS.moveSpeed) * (liquidDef ? liquidDef.speedMult : 1);
  if(player.dashing){
...
async function init(){
  try {
    ...
    const [[tileset, door, gun, bullet, ladder, barrel], , , , , , mapsRaw, laserFrames, barrierFrames, buttonShootFrames, [toggleOn, toggleOff], , splashFrames, explosionFrames, crateExplosionFrames] = await Promise.all([
      Promise.all([loadImage(TILESET_SRC), loadImage(DOOR_SRC), loadImage(GUN_SRC), loadImage(BULLET_SRC), loadImage(LADDER_SRC), loadImage(BARREL_SRC)]),
