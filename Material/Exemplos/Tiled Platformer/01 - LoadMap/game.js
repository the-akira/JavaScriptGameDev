...
// Sprite estático do jogador: imagem 32x48, personagem centralizado
// horizontalmente e com os pés perto da linha y=36 dentro do frame.
const SPRITE_W = 32;
const SPRITE_H = 48;
const SPRITE_FEET_Y = 36;
...
function spawnAtDoor(mapData){
  ...
  if(!d){ player.x = 20; player.y = 20; return; }
...
function drawPlayer(){
  const bob = player.onGround ? Math.sin(player.animT*14) * (Math.abs(player.vx)>2 ? 1 : 0) : 0;
  const feetX = player.x + player.w/2;
  const feetY = player.y + player.h + bob*0.4;

  // sombra no chão
  ctx.fillStyle = "rgba(0,0,0,0.35)";
  ctx.beginPath();
  ctx.ellipse(feetX, player.y + player.h + 1.5, player.w/2, 2, 0, 0, Math.PI*2);
  ctx.fill();
  ...
  ctx.drawImage(playerImg, -SPRITE_W/2, 0, SPRITE_W, SPRITE_H);
  ctx.restore();
}
...
async function init(){
  try {
    [[tilesetImg, doorImg, playerImg], MAPS_RAW] = await Promise.all([
      Promise.all([loadImage(TILESET_SRC), loadImage(DOOR_SRC), loadImage(PLAYER_SRC)]),
      loadAllMaps()
    ]);
  } catch(err){
    showLoadError(err);
