...
// Modo Matemática: troca a aparência gráfica normal por uma visão
// geométrica/didática (grade, hitboxes, vetores, área de visão dos
// inimigos, dados numéricos) — ver mathMode/drawMathWorld mais abaixo.
const MATH_MODE_KEY = "KeyM"; // liga/desliga o modo (master switch)
// Cada tecla abaixo alterna UMA camada de visualização, mas só tem
// efeito enquanto o Modo Matemática já está ligado (ver keydown).
const MATH_LAYER_KEYS = {
  KeyG: "grid",       // grade de coordenadas do mundo, em tiles
  KeyH: "hitboxes",   // caixas de colisão de jogador/inimigos (em vez de um pontinho)
  KeyF: "vectors",    // vetores de velocidade e gravidade
  KeyO: "vision",     // área de detecção e linha de visão dos inimigos
  KeyN: "numbers",    // painel numérico + rótulos de posição/velocidade
  KeyT: "trajectory"  // arco previsto do próximo pulo do jogador
};
...
// true = mostra a visão geométrica no lugar dos sprites (ver
// drawMathWorld); cada camada abaixo pode ser ligada/desligada à parte
// (ver MATH_LAYER_KEYS) enquanto o modo estiver ativo. Não afeta a
// física/lógica do jogo em nada — só o desenho.
...
const mathMode = {
  ...
  vision: true,
  numbers: true,
  trajectory: false // arco de pulo desligado por padrão — fica poluído se sempre visível
};
...
window.addEventListener('keydown', e => {
  ...
  if(e.code === MATH_MODE_KEY && !e.repeat){
    mathModeEnabled = !mathModeEnabled;
    console.log("Modo Matemática:", mathModeEnabled ? "LIGADO" : "DESLIGADO");
    MathModeUI.syncUI();
  }
  // Camadas individuais só reagem com o modo já ligado, pra essas letras
  // (G/H/F/O/N/T) não terem efeito nenhum durante o jogo normal.
  if(mathModeEnabled && !e.repeat && MATH_LAYER_KEYS[e.code]){
    const layer = MATH_LAYER_KEYS[e.code];
    mathMode[layer] = !mathMode[layer];
    console.log("[Modo Matemática] " + layer + ":", mathMode[layer]);
    MathModeUI.syncUI();
  }
...
/* ============================================================
   PAINEL DE CONTROLES DO MODO MATEMÁTICA (DOM em JS puro) — mesmo
   padrão do painel de CRT: um botão fixo na tela que abre uma janela
   com o liga/desliga geral e uma checkbox por camada (ver mathMode).
   ============================================================ */
const MathModeUI = (function(){
  let btnEl = null, panelEl = null, onoffEl = null;
  const checks = {};

  const LAYER_ORDER = ["grid", "hitboxes", "vectors", "vision", "numbers", "trajectory"];
  const LAYER_LABELS = {
    grid: "Grade de coordenadas (G)",
    hitboxes: "Hitboxes (H)",
    vectors: "Vetores de velocidade/gravidade (F)",
    vision: "Visão dos inimigos (O)",
    numbers: "Painel numérico (N)",
    trajectory: "Trajetória do pulo (T)"
  };
  ...
  function syncUI(){
    if(!panelEl) return;
    onoffEl.textContent = 'Modo Matemática: ' + (mathModeEnabled ? 'LIGADO' : 'DESLIGADO') + '  (tecla M)';
    onoffEl.classList.toggle('on', mathModeEnabled);
    btnEl.classList.toggle('on', mathModeEnabled);
    for(const name of LAYER_ORDER) checks[name].checked = mathMode[name];
  }
  function makeButton(text, onClick){
    const b = document.createElement('div');  // div (e não <button>) pra nunca roubar o foco do teclado
    b.className = 'mm-btn';
  ...
  function makeCheckRow(name){
    const row = document.createElement('label');
    row.className = 'mm-row';
    const input = document.createElement('input');
    input.type = 'checkbox';
    input.checked = mathMode[name];
    input.addEventListener('change', () => toggleLayer(name));
    const span = document.createElement('span');
    span.textContent = LAYER_LABELS[name];
  ...
  function buildUI(){
    const style = document.createElement('style');
    style.textContent = `
    #mmBtn{position:fixed;left:${MINIMAP_MARGIN}px;top:${MINIMAP_MARGIN + 88}px;z-index:30;width:35px;height:35px;
      display:flex;align-items:center;justify-content:center;font-size:15px;cursor:pointer;user-select:none;
      background:rgba(5,6,8,0.6);border:2px solid #2b2f3a;}
    #mmBtn.on{border-color:#3ce88c;}
    #mmPanel{position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);z-index:30;width:310px;max-height:90vh;overflow-y:auto;padding:10px 12px;
          background:rgba(5,6,8,0.78);border:2px solid #2b2f3a;font-family:monospace;user-select:none;}
    #mmPanel .mm-btn{text-align:center;padding:5px 8px;margin-bottom:10px;cursor:pointer;color:#dfe6f0;font:bold 12px monospace;border:2px solid #2b2f3a;}
    #mmPanel .mm-btn:hover{border-color:#8b93a6;}
    #mmPanel .mm-btn.on{color:#3ce88c;border-color:#3ce88c;}
    #mmPanel .mm-row{display:flex;align-items:center;gap:8px;margin:7px 0;cursor:pointer;color:#dfe6f0;font:bold 12px monospace;
      text-shadow:1px 1px 0 #000,-1px -1px 0 #000,1px -1px 0 #000,-1px 1px 0 #000;}
    #mmPanel .mm-row input{cursor:pointer;width:14px;height:14px;}
    `;
    document.head.appendChild(style);
    ...
    btnEl.id = 'mmBtn';
    btnEl.textContent = '🧮';
    btnEl.title = 'Painel do Modo Matemática (tecla M)';
    btnEl.addEventListener('click', togglePanel);
    ...
    onoffEl = makeButton('', () => setEnabled(!mathModeEnabled));
    panelEl.appendChild(onoffEl);

    for(const name of LAYER_ORDER) panelEl.appendChild(makeCheckRow(name));
  ...
  return { setEnabled, toggleLayer, togglePanel, syncUI };
})();
...
function updatePlatforms(dt){
  for(const p of livePlatforms){
    ...
    // p.vx/p.vy: velocidade "de verdade" (px/s), fixa por estado —
    // usada só pela seta do Modo Matemática. p.dx/p.dy (calculados no
    // fim da função) continuam sendo o deslocamento REAL deste frame
    // (dt-dependente), usados pra mover quem está em cima da
    // plataforma; não dá pra reaproveitar eles pra seta porque a
    // magnitude deles varia frame a frame com o dt (a seta ficava
    // "tremendo"/pulando de tamanho) e caem pra zero durante toda
    // pausa do elevador, fazendo a seta sumir e voltar de repente.
    p.vx = 0; p.vy = 0;
    ...
    if((p.kind === "elevator" || p.kind === "horizontal") && p.stops.length > 1){
      ...
      if(p.pauseT > 0){
        ...
        if(Math.abs(diff) <= p.speed * dt){
          ...
          p[axis] += dir * p.speed * dt;
          if(axis === "x") p.vx = dir * p.speed; else p.vy = dir * p.speed;
        }
      }
    } else if(p.kind === "circular"){
      const angularSpeed = Math.PI * 2 / p.period; // sentido horário
      p.angle += angularSpeed * dt;
      p.x = p.centerX + Math.cos(p.angle) * p.radius - p.w/2;
      p.y = p.centerY + Math.sin(p.angle) * p.radius - p.h/2;
      // Velocidade tangencial exata (derivada da posição), não a
      // diferença entre dois frames — fica suave em qualquer FPS.
      p.vx = -Math.sin(p.angle) * p.radius * angularSpeed;
      p.vy = Math.cos(p.angle) * p.radius * angularSpeed;
    }
...
/* ============================================================
   MODO MATEMÁTICA — visão geométrica/didática do jogo, desenhada no
   lugar dos sprites normais (ver mathModeEnabled/mathMode acima).
   Nenhuma função aqui mexe em física ou estado de jogo: só leem as
   mesmas variáveis (player, enemies, currentMap, PHYS...) que o
   desenho normal já usa, e desenham formas/linhas/texto em cima.
   ============================================================ */
// Fator de escala usado por TODAS as setas de velocidade do Modo
// Matemática (jogador, inimigos, plataformas) — não é 1:1 com px/s,
// só pra caber na tela; mantido único pra toda seta usar a mesma régua.
const MATH_VSCALE = 0.15;

const MATH_COLORS = {
  bg: "#030a08",
  gridMinor: "rgba(60,220,140,0.12)",
  gridMajor: "rgba(60,220,140,0.30)",
  solid: "#3ce88c",
  ladder: "#e8c14c",
  liquidFill: "rgba(70,170,255,0.16)",
  liquidLine: "rgba(70,170,255,0.5)",
  oneWay: "#7fd6ff",
  laserOn: "#ff5a5a",
  laserOff: "rgba(255,90,90,0.25)",
  button: "#d17dff",
  crate: "#e8c14c",
  crateFill: "rgba(232,193,76,0.12)",
  barrel: "#ff8c3c",
  barrelFill: "rgba(255,140,60,0.12)",
  player: "#38f0ff",
  playerFill: "rgba(56,240,255,0.18)",
  enemy: "#ff4d6d",
  enemyFill: "rgba(255,77,109,0.18)",
  bullet: "#ffffff",
  velocity: "#ffe066",
  gravity: "#8f8fff",
  sightOk: "#ff4d6d",
  sightBlocked: "rgba(255,255,255,0.3)",
  door: "#5fb8d1",
  secret: "#4fd1c5",
  itemDefault: "#e8c14c",
  lootDefault: "#e05a5a",
  text: "#9dffcf",
  explosionBarrel: "rgba(255,140,60,0.6)",
  explosionCrate: "rgba(232,193,76,0.6)"
};

// Seta simples (linha + ponta), em coordenadas de mundo, usada por todo
// vetor desenhado (velocidade, gravidade, bala). dx/dy já vêm na escala
// visual final (ver chamadas) — a função só desenha, não escala.
function drawMathArrow(x, y, dx, dy, color){
...
// Grade de coordenadas do mundo (em tiles), com linhas mais grossas a
// cada 5 tiles e o número da coluna/linha nelas (só quando "numbers"
// também está ligado, pra não competir com os rótulos das entidades).
function drawMathGrid(x0, y0, x1, y1){
  const colStart = Math.floor(x0 / TILE), colEnd = Math.ceil(x1 / TILE);
  const rowStart = Math.floor(y0 / TILE), rowEnd = Math.ceil(y1 / TILE);

  ctx.lineWidth = 1 / ZOOM;
  ctx.beginPath();
  for(let col = colStart; col <= colEnd; col++){
    if(col % 5 === 0) continue;
    ctx.moveTo(col*TILE, y0); ctx.lineTo(col*TILE, y1);
  }
  for(let row = rowStart; row <= rowEnd; row++){
    if(row % 5 === 0) continue;
    ctx.moveTo(x0, row*TILE); ctx.lineTo(x1, row*TILE);
  }
  ctx.strokeStyle = MATH_COLORS.gridMinor;
  ctx.stroke();
  ...
  ctx.strokeStyle = MATH_COLORS.gridMajor;
  ctx.stroke();

  if(mathMode.numbers){
    ctx.fillStyle = MATH_COLORS.gridMajor;
    ctx.font = "5px monospace";
    ctx.textAlign = "left";
    for(let col = colStart; col <= colEnd; col++){
      if(col % 5 !== 0) continue;
      ctx.fillText(String(col), col*TILE + 1, y0 + 6);
    }
    for(let row = rowStart; row <= rowEnd; row++){
      if(row % 5 !== 0) continue;
      ctx.fillText(String(row), x0 + 1, row*TILE - 1);
    }
  }
}

// Tiles sólidos (chão/parede — mesma checagem usada pela colisão de
// verdade, isSolidTile) e colunas de escada, dentro da área visível.
// Gatilhos de passagem secreta (objectgroup "Secret"): a decoração
// escondida (secretTiles) já aparece automaticamente em drawMathTiles
// como um bloco sólido comum (via isSolidTile), disfarçada igual no
// jogo normal — de propósito, pra não entregar o segredo. Mas os
// próprios GATILHOS (liveSecretTriggers) deviam ficar sempre visíveis
// como uma pista (é o que acontece no modo normal, ver drawSecrets) e
// no Modo Matemática eles ficavam sem NENHUMA marca: o "bulletCollision"
// também é sólido, então se perdia no meio do bloco cinza comum, e o
// "playerCollision" nem é sólido, então não aparecia nada ali.
function drawMathSecrets(){
  // Some no mesmo instante que o bloco sólido (drawMathTiles/isSolidTile)
  // — ambos caem junto com secretOpened. Não espera secretDissolving:
  // essa janela só existe pro desenho com a arte real do tile (efeito
  // de "esfarelar" do modo normal), que o Modo Matemática não reproduz.
  if(secretOpened) return;
  ctx.lineWidth = 1 / ZOOM;
  for(const trig of liveSecretTriggers){
    const isBullet = trig.trigger === "bulletCollision";
    ctx.fillStyle = "rgba(79,209,197,0.22)";
    ctx.fillRect(trig.x, trig.y, trig.w, trig.h);
    ctx.strokeStyle = MATH_COLORS.secret;
    ctx.setLineDash(isBullet ? [2,2] : []); // tracejado = precisa atirar; sólido = basta encostar
    ctx.strokeRect(trig.x+0.5, trig.y+0.5, trig.w-1, trig.h-1);
    ctx.setLineDash([]);
    if(mathMode.numbers){
      ctx.fillStyle = MATH_COLORS.secret;
      ctx.font = "5px monospace";
      ctx.textAlign = "center";
      ctx.fillText(isBullet ? "segredo: atirar" : "segredo: tocar", trig.x + trig.w/2, trig.y - 2);
    }
  }
}

function drawMathTiles(x0, y0, x1, y1){
  const colStart = Math.max(0, Math.floor(x0/TILE));
  const colEnd = Math.min(currentMap.width - 1, Math.ceil(x1/TILE));
  const rowStart = Math.max(0, Math.floor(y0/TILE));
  const rowEnd = Math.min(currentMap.height - 1, Math.ceil(y1/TILE));

  ctx.lineWidth = 1 / ZOOM;
  for(let row = rowStart; row <= rowEnd; row++){
    for(let col = colStart; col <= colEnd; col++){
      const wx = col*TILE, wy = row*TILE;
      if(isSolidTile(currentMap, wx + TILE/2, wy + TILE/2)){
        ctx.fillStyle = "rgba(60,232,140,0.15)";
        ctx.fillRect(wx, wy, TILE, TILE);
        ctx.strokeStyle = MATH_COLORS.solid;
        ctx.strokeRect(wx+0.5, wy+0.5, TILE-1, TILE-1);
      } else if(currentMap.ladder && tileAt(currentMap.ladder, currentMap, col, row) !== 0){
        ctx.setLineDash([2,2]);
        ctx.strokeStyle = MATH_COLORS.ladder;
        ctx.strokeRect(wx+2, wy, TILE-4, TILE);
        ctx.setLineDash([]);
      }
    }
  }
...
function drawMathLiquids(){
  for(const l of liveLiquids){
    ctx.fillStyle = MATH_COLORS.liquidFill;
    ctx.fillRect(l.x, l.y, l.w, l.h);
    ctx.strokeStyle = MATH_COLORS.liquidLine;
    ctx.lineWidth = 1 / ZOOM;
    ctx.strokeRect(l.x+0.5, l.y+0.5, l.w-1, l.h-1);
  }
}

// Plataformas one-way: contorno tracejado (pra diferenciar de tile
// sólido normal) + vetor de deslocamento quando estão em movimento
// (elevador/circular/horizontal).
function drawMathPlatforms(){
  ctx.lineWidth = 1 / ZOOM;
  for(const p of livePlatforms){
    const pos = snapToCam(p.x, p.y);
    ctx.setLineDash([3,2]);
    ctx.strokeStyle = MATH_COLORS.oneWay;
    ctx.strokeRect(pos.x+0.5, pos.y+0.5, p.w-1, p.h-1);
    ctx.setLineDash([]);
    if(mathMode.vectors && (p.vx || p.vy)){
      drawMathArrow(pos.x + p.w/2, pos.y + p.h/2, p.vx*MATH_VSCALE, p.vy*MATH_VSCALE, MATH_COLORS.velocity);
    }
  }
}

function drawMathCratesBarrels(){
  ctx.lineWidth = 1 / ZOOM;
  for(const c of crates){
    ctx.fillStyle = MATH_COLORS.crateFill;
    ctx.fillRect(c.x, c.y, c.w, c.h);
    ctx.strokeStyle = MATH_COLORS.crate;
    ctx.strokeRect(c.x+0.5, c.y+0.5, c.w-1, c.h-1);
  }
  for(const b of barrels){
    ctx.fillStyle = MATH_COLORS.barrelFill;
    ctx.fillRect(b.x, b.y, b.w, b.h);
    ctx.strokeStyle = MATH_COLORS.barrel;
    ctx.strokeRect(b.x+0.5, b.y+0.5, b.w-1, b.h-1);
    if(b.fuseTimer !== null){
      // raio da reação em cadeia — só enquanto o barril está "contagiado"
      ctx.strokeStyle = "rgba(255,140,60,0.4)";
      ctx.beginPath();
      ctx.arc(b.x+b.w/2, b.y+b.h/2, BARREL_CHAIN_RADIUS, 0, Math.PI*2);
      ctx.stroke();
    }
  }
}
...
function drawMathTriggers(){
  ctx.lineWidth = 1 / ZOOM;
  for(const l of liveLasers){
    // "openness" é o progresso REAL da animação (0 = fechada/bloqueando,
    // 1 = totalmente aberta), tirado direto do frame atual — não do
    // "blocking", que só muda quando a animação termina. Assim a cor
    // vai desbotando aos poucos enquanto abre/fecha, em vez de pular
    // de um estado pro outro no fim (o que dava a impressão de que
    // nada estava acontecendo durante a animação).
    const frameCount = (l.kind === "barrier" ? BARRIER_SRC : LASER_SRC).length;
    const openness = l.frame / (frameCount - 1);
    const animating = l.frame !== l.targetFrame;

    // A cor de perigo (tint) segue o estado REAL de colisão, não o
    // sprite: ao fechar (l.on = true) ela já bloqueia desde o clique
    // no botão (ver setDoorOn), então o tint já vai direto pro
    // "fechada" final, sem esperar a animação. Só ao abrir é que
    // continua bloqueando até o frame terminar de verdade — por isso
    // só nesse sentido o tint acompanha o progresso do frame aos poucos.
    const tintOpenness = l.on ? 0 : openness;
    const fillAlpha = 0.18 + (0.05 - 0.18) * tintOpenness;
    const strokeAlpha = 1 + (0.25 - 1) * tintOpenness;
    ctx.fillStyle = `rgba(255,90,90,${fillAlpha.toFixed(3)})`;
    ctx.fillRect(l.x, l.y, l.w, l.h);
    ctx.strokeStyle = `rgba(255,90,90,${strokeAlpha.toFixed(3)})`;
    ctx.strokeRect(l.x+0.5, l.y+0.5, l.w-1, l.h-1);

    // Barra de progresso: pura geometria, some parte da própria
    // aparência do Modo Matemática (não depende da camada "numbers"),
    // mostra a fração já percorrida entre fechada e aberta.
    if(animating){
      const barH = Math.max(1, Math.min(3, l.h * 0.08));
      ctx.fillStyle = MATH_COLORS.text;
      ctx.fillRect(l.x, l.y + l.h - barH, l.w * openness, barH);
    }

    if(mathMode.numbers){
      const pct = Math.round(openness * 100);
      const label = animating
        ? ((l.targetFrame > l.frame ? "abrindo " : "fechando ") + pct + "%")
        : (l.blocking ? "fechada" : "aberta");
      ctx.fillStyle = MATH_COLORS.text;
      ctx.font = "5px monospace";
      ctx.textAlign = "center";
      ctx.fillText(label, l.x + l.w/2, l.y - 2);
    }
  }
  for(const b of liveButtons){
    ctx.strokeStyle = b.disabled ? "rgba(209,125,255,0.25)" : MATH_COLORS.button;
    ctx.strokeRect(b.x+0.5, b.y+0.5, b.w-1, b.h-1);

    // Botão "shoot": indicador de "vida" (tiros restantes até
    // desligar de vez), já que no modo normal isso só aparece pelo
    // sprite (buttonShootImgs[b.hits] — ver drawButtons). Barra pura
    // geometria (independe de mathMode.numbers), mesmo esquema visual
    // da barra de progresso dos lasers logo acima.
    if(b.kind === "shoot"){
      const hitsLeft = BUTTON_HITS_TO_DISABLE - b.hits;
      const pct = Math.max(0, hitsLeft / BUTTON_HITS_TO_DISABLE);
      const barW = b.w, barH = Math.max(1, Math.min(3, b.h * 0.12));
      const barX = b.x, barY = b.y - barH - 2;

      ctx.fillStyle = "rgba(0,0,0,0.55)";
      ctx.fillRect(barX - 1, barY - 1, barW + 2, barH + 2);
      ctx.fillStyle = "#2a2e38";
      ctx.fillRect(barX, barY, barW, barH);
      ctx.fillStyle = b.disabled ? "rgba(209,125,255,0.4)"
        : (pct > 0.5 ? "#5fd15f" : (pct > 0.25 ? "#e8c14c" : "#e05a5a"));
      ctx.fillRect(barX, barY, barW * pct, barH);

      if(mathMode.numbers){
        const label = b.disabled ? "desligado" : (hitsLeft + "/" + BUTTON_HITS_TO_DISABLE);
        ctx.fillStyle = MATH_COLORS.text;
        ctx.font = "5px monospace";
        ctx.textAlign = "center";
        ctx.fillText(label, b.x + b.w/2, barY - 2);
      }
    }
  }
}

// Caixa de detecção (ENEMY_SIGHT_RANGE x ENEMY_SIGHT_VERT) de um
// inimigo, e a linha até o jogador quando ele está dentro dela —
// verde se a linha de visão está livre, cinza se algo bloqueia
// (mesma checagem hasLineOfSight usada pela IA de verdade).
function drawMathVision(enemy){
  const range = enemy.state === "alert" ? ENEMY_SIGHT_RANGE + ENEMY_ALERT_RANGE_BONUS : ENEMY_SIGHT_RANGE;
  const cx = enemy.x + enemy.w/2, cy = enemy.y + enemy.h/2;
  const px = player.x + player.w/2, py = player.y + player.h/2;
  const inBox = Math.abs(px-cx) <= range && Math.abs(py-cy) <= ENEMY_SIGHT_VERT;

  ctx.lineWidth = 1 / ZOOM;
  ctx.setLineDash([2,2]);
  ctx.strokeStyle = enemy.state === "alert" ? "rgba(255,77,109,0.55)" : "rgba(255,255,255,0.18)";
  ctx.strokeRect(cx-range, cy-ENEMY_SIGHT_VERT, range*2, ENEMY_SIGHT_VERT*2);
  ctx.setLineDash([]);

  if(inBox){
    const clear = hasLineOfSight(enemy, player);
    ctx.strokeStyle = clear ? MATH_COLORS.sightOk : MATH_COLORS.sightBlocked;
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(px, py);
    ctx.stroke();
...
// Hitbox (ou um pontinho, se a camada "hitboxes" estiver desligada) +
// marca de direção (facing) + letra identificando o tipo.
function drawMathEntity(entity, color, fill, label){
  const pos = snapToCam(entity.x, entity.y);
  const cx = pos.x + entity.w/2, cy = pos.y + entity.h/2;
  ctx.lineWidth = 1 / ZOOM;
  if(mathMode.hitboxes){
    ctx.fillStyle = fill;
    ctx.fillRect(pos.x, pos.y, entity.w, entity.h);
    ctx.strokeStyle = color;
    ctx.strokeRect(pos.x+0.5, pos.y+0.5, entity.w-1, entity.h-1);
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(cx + entity.facing*(entity.w/2+3), cy);
    ctx.stroke();
  } else {
    ctx.fillStyle = color;
    ctx.beginPath();
  ...
  ctx.textAlign = "center";
  ctx.fillText(label, cx, pos.y - 2);
}

// Vetor de velocidade (amarelo) e, se estiver no ar, vetor de
// gravidade (roxo) puxando pra baixo — ambos escalados só pra caber
// na tela (não é 1:1 com px/s, ver VSCALE).
function drawMathVectors(entity, gravityVal){
  const pos = snapToCam(entity.x, entity.y);
  const cx = pos.x + entity.w/2, cy = pos.y + entity.h/2;
  if(Math.abs(entity.vx) > 1 || Math.abs(entity.vy) > 1){
    drawMathArrow(cx, cy, entity.vx*MATH_VSCALE, entity.vy*MATH_VSCALE, MATH_COLORS.velocity);
  }
  if(!entity.onGround && !entity.climbing){
    drawMathArrow(cx, cy, 0, gravityVal*MATH_VSCALE*0.4, MATH_COLORS.gravity);
  }
}

// Arco previsto do próximo pulo (se no chão) ou da queda atual (se no
// ar), simulando frame a frame com as mesmas constantes de PHYS —
// para no primeiro tile sólido que encontrar. Puramente ilustrativo:
// não considera vento/friction horizontal nem colisão de caixas.
function drawMathTrajectory(){
  const startVx = player.onGround ? player.facing * PHYS.moveSpeed : player.vx;
  let vy = player.onGround ? PHYS.jumpVel : player.vy;
  let x = player.x + player.w/2, y = player.y + player.h/2;
  ...
  for(let t=0; t<2; t+=STEP){
    x += startVx*STEP;
    vy = Math.min(PHYS.maxFall, vy + PHYS.gravity*STEP);
    y += vy*STEP;
    if(isSolidTile(currentMap, x, y)) break;
    ctx.lineTo(x, y);
  }
  ctx.stroke();
  ctx.setLineDash([]);
...
function drawMathBullets(){
  for(const b of bullets){
    ctx.fillStyle = MATH_COLORS.bullet;
    ctx.beginPath();
    ctx.arc(b.x, b.y, 1.5, 0, Math.PI*2);
    ctx.fill();
    if(mathMode.vectors) drawMathArrow(b.x, b.y, b.vx*0.05, b.vy*0.05, MATH_COLORS.bullet);
  }
}

// Raio de dano das explosões ativas (barril/caixa), em vez da nuvem de
// partículas — mostra a área numérica real (BARREL_EXPLOSION_RADIUS_*)
// usada pelo dano em vez de só um efeito visual.
function drawMathExplosions(){
  ctx.lineWidth = 1 / ZOOM;
  for(const e of explosions){
    const isBarrel = e.kind === "barrel";
    const rx = isBarrel ? BARREL_EXPLOSION_RADIUS_X : 30;
    const ry = isBarrel ? BARREL_EXPLOSION_RADIUS_Y : 30;
    ctx.strokeStyle = isBarrel ? MATH_COLORS.explosionBarrel : MATH_COLORS.explosionCrate;
    ctx.beginPath();
...
// Zonas de porta (objectgroup "Doors" — a área REAL que dispara a troca
// de mapa, não só o desenho decorativo do tile) + rótulo com o destino,
// lido direto de Origin/Destiny/DestinyDoor do .tmx.
function drawMathDoors(){
  ctx.lineWidth = 1 / ZOOM;
  for(const d of currentMap.doors){
    ctx.fillStyle = "rgba(95,184,209,0.18)";
    ctx.fillRect(d.x, d.y, d.width, d.height);
  }
  ctx.setLineDash([3,2]);
  ctx.strokeStyle = MATH_COLORS.door;
  for(const d of currentMap.doors) ctx.strokeRect(d.x+0.5, d.y+0.5, d.width-1, d.height-1);
  ctx.setLineDash([]);
  if(mathMode.numbers){
    ctx.fillStyle = MATH_COLORS.door;
    ctx.font = "5px monospace";
    ctx.textAlign = "center";
    for(const d of currentMap.doors){
      const label = d.destiny ? ("\u2192 " + d.destiny + (d.destinyDoor ? ("#" + d.destinyDoor) : "")) : "porta";
      ctx.fillText(label, d.x + d.width/2, d.y - 2);
    }
  }
}

// Losango contornado + rótulo, usado tanto pros itens do mapa
// (liveItems) quanto pro loot dropado por caixa (loots) — em vez do
// ícone/sprite de cada um.
function drawMathPickup(x, y, w, h, color, label){
  // rx/ry separados (em vez de um único r = max(w,h)/2): itens mais
  // largos que altos (ou vice-versa) tinham o losango "vazando" pra
  // fora da própria caixa do item nesse eixo — pra baixo, afundando
  // no tile embaixo, quando w > h. Assim o losango fica sempre
  // inscrito exatamente no retângulo real do item.
...
function drawMathItems(){
  for(const it of liveItems){
    drawMathPickup(it.x, it.y, it.w, it.h, it.def.color || MATH_COLORS.itemDefault, it.def.name);
  }
}

function drawMathLoots(){
  for(const l of loots){
    const label = l.def && l.def.heal ? ("+" + l.def.heal) : "loot";
    drawMathPickup(l.x, l.y, l.w, l.h, MATH_COLORS.lootDefault, label);
  }
}

function drawMathLabel(entity, text){
  const pos = snapToCam(entity.x, entity.y);
  ctx.fillStyle = MATH_COLORS.text;
  ctx.font = "5px monospace";
  ctx.textAlign = "center";
  ctx.fillText(text, pos.x + entity.w/2, pos.y + entity.h + 7);
}

// Monta o mundo inteiro em modo geométrico, na ordem que faz sentido
// visualmente (fundo -> tiles -> objetos estáticos -> gatilhos -> área
// de visão -> personagens -> vetores -> projéteis -> números por cima
// de tudo). Só os balões de interação (drawDoorPrompt/drawButtonPrompt,
// o "E" que aparece perto de porta/botão) continuam com o desenho
// normal: são interface de gameplay, não a "aparência gráfica" do
// mundo que o Modo Matemática troca.
function drawMathWorld(cam){
  const viewX0 = cam.xInt, viewY0 = cam.yInt;
  const viewX1 = viewX0 + canvas.width/ZOOM, viewY1 = viewY0 + canvas.height/ZOOM;

  ctx.fillStyle = MATH_COLORS.bg;
  ctx.fillRect(viewX0, viewY0, viewX1-viewX0, viewY1-viewY0);

  if(mathMode.grid) drawMathGrid(viewX0, viewY0, viewX1, viewY1);
  drawMathTiles(viewX0, viewY0, viewX1, viewY1);
  drawMathSecrets();
  drawMathLiquids();
  drawMathPlatforms();
  drawMathCratesBarrels();
  drawMathTriggers();
  drawMathDoors();
  drawDoorPrompt();
  drawButtonPrompt();
  ...
  if(mathMode.vision) for(const enemy of enemies) drawMathVision(enemy);

  for(const enemy of enemies){
    drawMathEntity(enemy, MATH_COLORS.enemy, MATH_COLORS.enemyFill, "E");
    drawEnemyHealthBar(enemy);
  }
  drawMathEntity(player, MATH_COLORS.player, MATH_COLORS.playerFill, "P");
  drawPlayerEffects();

  if(mathMode.vectors){
    for(const enemy of enemies) drawMathVectors(enemy, ENEMY_PHYS.gravity);
    drawMathVectors(player, PHYS.gravity);
  }
  if(mathMode.trajectory) drawMathTrajectory();
  ...
  if(mathMode.numbers){
    for(const enemy of enemies){
      drawMathLabel(enemy, `\u0394x:${Math.round(player.x-enemy.x)} \u0394y:${Math.round(player.y-enemy.y)}`);
    }
    drawMathLabel(player, `v(${Math.round(player.vx)}, ${Math.round(player.vy)})`);
  }
}

// Painel numérico de canto (posição/velocidade/estado do jogador +
// constantes de física atuais) — desenhado em espaço de TELA (fora do
// ctx.scale/translate da câmera), chamado depois do ctx.restore() em
// render(). Sempre mostra um selo indicando o modo ligado, mesmo que
// a camada "numbers" esteja desligada.
function drawMathModeIndicator(){
  ctx.save();
  ctx.font = "10px monospace";
  ctx.fillStyle = MATH_COLORS.solid;
  ctx.textAlign = "right";
  ctx.textBaseline = "top";
  ctx.fillText("Modo Matemática (M)", canvas.width - 16, 80);
  ctx.restore();

  if(!mathMode.numbers) return;

  const lines = [
    `Jogador   x:${player.x.toFixed(1)}  y:${player.y.toFixed(1)}   vx:${player.vx.toFixed(1)}  vy:${player.vy.toFixed(1)}`,
    `Física    gravidade:${PHYS.gravity}px/s\u00b2   moveSpeed:${PHYS.moveSpeed}px/s   jumpVel:${PHYS.jumpVel}px/s`,
    `Estado    onGround:${player.onGround}  crouching:${player.crouching}  climbing:${player.climbing}  emLíquido:${player.inLiquid}`
  ];
  ctx.save();
  ...
  boxW += pad*2;
  const boxH = pad*1.5 + lineH*lines.length;
  ctx.fillStyle = "rgba(3,10,8,0.78)";
  ctx.fillRect(8, 8 + 30, boxW, boxH);
  ctx.strokeStyle = "rgba(60,220,140,0.5)";
  ctx.strokeRect(8.5, 8.5 + 30, boxW-1, boxH-1);
  ctx.fillStyle = MATH_COLORS.text;
  lines.forEach((line,i) => ctx.fillText(line, 8+pad, 8+30+pad+i*lineH));
  ctx.restore();
}
...
function render(){
  ...
  // No Modo Matemática o jogador já é um retângulo/ponto (não um
  // sprite), então esse redesenho — que existe só pra travar a
  // paridade de linha do sprite pro efeito CRT — não se aplica; a
  // plataforma de carona continua sendo travada normalmente.
  ...
  if(mathModeEnabled) drawMathModeIndicator();
