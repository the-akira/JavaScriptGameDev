...
// Sprite simétrico (sem espaço vazio sobrando), desenhado centralizado
// no centro da caixa — ver spawnExplosion/drawExplosions.
...
function parseTMX(xmlText){
  ...
  return { width, height, layers, doors, enemySpawns, crateSpawns, platformSpawns, laserSpawns, buttonSpawns };
}
...
// x/y: centro da caixa destruída (sprite simétrico, sem espaço vazio a
// compensar — ver drawExplosions).
...
function updatePlayer(dt){
  ...
  const currentMaxSpeed = player.crouching ? PHYS.moveSpeed * CROUCH_SPEED_MULT : PHYS.moveSpeed;
  if(player.dashing){
...
function getShoulderWorld(entity){
  ...
  const feetY = entity.y + entity.h;
  const feetYOffset = entity.crouching ? CROUCH_SPRITE_FEET_Y : SPRITE_FEET_Y;
  const attach = entity.crouching ? ARM_ATTACH_CROUCH : ARM_ATTACH;
...
function drawExplosions(){
  ...
  for(const e of explosions){
    ...
    if(!img) continue;
    // Sprite simétrico (sem espaço vazio sobrando) — desenhado
...
function drawCharacter(entity, animsSet){
  ...
  const spriteH = entity.crouching ? CROUCH_SPRITE_H : SPRITE_H;
  const feetYOffset = entity.crouching ? CROUCH_SPRITE_FEET_Y : SPRITE_FEET_Y;

  ctx.save();
  ctx.translate(drawFeet.x, drawFeet.y - feetYOffset);
  ctx.scale(entity.facing, 1);
  ctx.drawImage(anim.img, sx, 0, SPRITE_W, spriteH, -SPRITE_W/2, 0, SPRITE_W, spriteH);
  ctx.restore();
}
...
function render(){
  ...
  // Agachado, o sprite (32px) é mais alto que a hitbox (14px) usada
  // pra caber no vão de 1 tile — então a cabeça pode visualmente
  // avançar por cima do tile do teto. Redesenhando a camada de tiles
  // sólidos por cima só nesse caso, qualquer parte do personagem que
  // esteja "dentro" de um tile fica coberta por ele (tiles vazios
  // continuam transparentes, não escondem nada fora dessas células).
  if(player.crouching) drawTileLayer(currentMap.platforms, currentMap);

  // Depois de qualquer redesenho da camada de tiles acima (inclusive o
  // do agachar) pra garantir que a explosão da caixa nunca fique
  // escondida atrás de um tile de plataforma.
  drawExplosions();
...
function loop(t){
  ...
  if(!gameStarted){
    ...
    updateExplosions(dt); // idem (ex.: morreu bem na hora que uma caixa explodiu)
    updatePlayer(dt);
    if(!gameOver){
...
async function init(){
  try {
    ...
    const [[tileset, door, gun, bullet, ladder], , , , , , mapsRaw, laserFrames, barrierFrames, buttonShootFrames, [toggleOn, toggleOff], crateExplosionFrames] = await Promise.all([
