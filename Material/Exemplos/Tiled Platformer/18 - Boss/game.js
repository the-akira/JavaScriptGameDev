...
function parseTMX(xmlText){
  ...
  return { width, height, layers, doors, enemySpawns, crateSpawns, barrelSpawns, secretTiles, secretTriggers, tilesFirstGid, tilesColumns, platformSpawns, laserSpawns, buttonSpawns, liquidSpawns };
}
...
function fireBulletFrom(entity, owner){
  const def = enemyDefFor(entity);
  const muzzleOffset = def ? def.muzzleOffset : MUZZLE_OFFSET;
  ...
  bullets.push({
    ...
    img: def ? def.bulletImg : bulletImg
  });
}
...
function drawCharacter(entity, animsSet){
  ...
  ctx.drawImage(anim.img, sx, 0, spriteW, spriteH, -spriteW/2, 0, spriteW, spriteH);
  ctx.restore();
}
...
function drawArm(entity){
  ...
  const def = enemyDefFor(entity);
  const img = def ? def.armImg : gunImg;
  if(!img) return;
  const armW = def ? def.armW : ARM_W;
  const armH = def ? def.armH : ARM_H;
  const pivot = def ? def.armPivot : ARM_PIVOT;
  const shoulder = getShoulderWorld(entity);
  ...
  ctx.scale(entity.facing, 1);
  ctx.drawImage(img, 0, 0, armW, armH, -pivot.x, -pivot.y, armW, armH);
  ctx.restore();
}
...
function drawBullets(){
  for(const b of bullets){
    ...
    if(!img) continue;
    ctx.drawImage(img, b.x - BULLET_W/2, b.y - BULLET_H/2, BULLET_W, BULLET_H);
  }
}
