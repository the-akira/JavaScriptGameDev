class Grid {
    constructor(ctx, step, color, drawCoordinates, mapWidth, mapHeight) {
        this.ctx = ctx;
        this.step = step;
        this.color = color;
    ...
    draw() {
        ...
        this.ctx.beginPath();
        for (let x = 0; x <= this.mapWidth; x += this.step) {
            this.ctx.moveTo(x, 0);
        ...
        for (let y = 0; y <= this.mapHeight; y += this.step) {
            this.ctx.moveTo(0, y);
...
class Camera {
    ...
    follow(target) {
        ...
        this.y = Math.min(Math.max(0, this.y), this.mapHeight - this.viewportHeight);
    }

    apply(ctx) {
        // Aplica o deslocamento da câmera ao contexto de desenho
        ctx.translate(-this.x, -this.y);
    }

    clear(ctx) {
        // Limpa o deslocamento da câmera aplicado ao contexto de desenho
...
class Game {
    constructor() {
        ...
        this.mapWidth = 1100; // Largura do mapa
        this.mapHeight = 950; // Altura do mapa
        this.grid = new Grid(this.ctx, 10, '#bdbdbd', false, this.mapWidth, this.mapHeight);
        this.camera = new Camera(this.canvas, this.mapWidth, this.mapHeight);
        this.gridWithCoordinates = new Grid(this.ctx, 50, '#545454', true, this.mapWidth, this.mapHeight);
    ...
    animate() {
        ...
        // Limpa o deslocamento da câmera após o desenho
    ...
    moveRectangle() {
        ...
        if (newX >= 0 && newX + this.player.width <= this.mapWidth) {
            this.player.x = newX;
        }
        if (newY >= 0 && newY + this.player.height <= this.mapHeight) {
            this.player.y = newY;
        }
    }
    ...
    startIntervals() {
        ...
        const circleIntervalId = setInterval(() => {
            this.objects.push(new Circle(this.ctx, this.mapWidth, Math.floor(Math.random() * this.mapHeight), 20, "#ed5168"));
        }, 5000);
        ...
        const triangleIntervalId = setInterval(() => {
            this.objects.push(new Triangle(this.ctx, Math.floor(Math.random() * this.mapWidth), 0, 50, 50, "#5edb86"));
        }, 5000);
