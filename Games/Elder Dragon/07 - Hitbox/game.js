...
fireballSound.volume = 0.08;

const themeMusic = new Audio('audio/Dvorak.mp3');
...
class Serpent {
    constructor(y, speed, numFrames) {
        ...
        this.width = 110; // Defina um tamanho inicial
        this.height = 112; 
        this.hitbox = {
            x: this.x + 10,
            y: this.y + 25,
        ...
        for (let i = 0; i < numFrames; i++) {
            this.frames[i].src = `assets/serpent${i + 1}.png`;
        }
...
function spawnSerpent() {
    let newSerpent = new Serpent(Math.random() * (canvas.height - 100), 3, 6);
    serpents.push(newSerpent);
}
...
class Firewave {
    ...
    checkCollision(fireballs, serpent) {
        // Verificar colisão com as Fireballs
        for (let i = 0; i < fireballs.length; i++) {
            ...
            if (
                ...
                // Se houver colisão, remover a Fireball
                fireballs.splice(i, 1);
                i--; // Reduzir o índice para compensar a remoção do elemento do array
                score++;
        ...
        for (let i = 0; i < serpents.length; i++) {
            let serpent = serpents[i];
            // Verificar se há colisão entre os retângulos delimitadores
            if (
                this.x < serpent.hitbox.x + serpent.hitbox.width &&
                this.x + this.width > serpent.hitbox.x &&
                this.y < serpent.hitbox.y + serpent.hitbox.height &&
                this.y + this.height > serpent.hitbox.y
            ) {
...
class Dragon {
    constructor(x, y, speed) {
        ...
        this.hitbox = {
            x: this.x + 20, // Ajuste a posição X da hitbox
            y: this.y + 45, // Ajuste a posição Y da hitbox
            width: 110 - 20,     // Largura da hitbox
            height: 110 - 70// Altura da hitbox
        };
    }
    ...
    updateHitbox() {
        // Atualizar a posição da hitbox conforme o dragão se move
    ...
    checkCollision(fireballs, serpents) {
        this.updateHitbox(); // Atualizar a posição da hitbox

        // Verificar colisão com fireballs
        for (let i = 0; i < fireballs.length; i++) {
            let fireball = fireballs[i];
            if (
                this.hitbox.x < fireball.x + fireball.width &&
                this.hitbox.x + this.hitbox.width > fireball.x &&
                this.hitbox.y < fireball.y + fireball.height &&
                this.hitbox.y + this.hitbox.height > fireball.y
            ) {
                // Se houver colisão, faça algo, por exemplo, reduza a vida do jogador
                gameState = GameState.telaInicial;
                break; // Não precisa verificar mais colisões se o jogador já colidiu
            }
        }

        // Verificar colisão com serpents
        for (let i = 0; i < serpents.length; i++) {
            let serpent = serpents[i];

            if (
                this.hitbox.x < serpent.hitbox.x + serpent.hitbox.width &&
                this.hitbox.x + this.hitbox.width > serpent.hitbox.x &&
                this.hitbox.y < serpent.hitbox.y + serpent.hitbox.height &&
                this.hitbox.y + this.hitbox.height > serpent.hitbox.y
            ) {
                // Se houver colisão, faça algo, por exemplo, reduza a vida do jogador
                gameState = GameState.telaInicial;
                break; // Não precisa verificar mais colisões se o jogador já colidiu
...
function drawFirewaveCooldownBar() {
    ...
    ctx.fillRect(10, 10, barFillWidth, barHeight);
}
...
function draw() {
    ...
    if (gameState === GameState.jogando) {
        ...
        player.checkCollision(fireballs, serpents);
        drawCooldownBar();
        ...
        firewaves.forEach((firewave, index) => {
            ...
            firewave.checkCollision(fireballs, serpents);
