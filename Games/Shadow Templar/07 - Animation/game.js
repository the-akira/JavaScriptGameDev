...
class Player {
    constructor(game) {
        this.game = game;
        this.width = 40; // Largura do jogador
        this.height = 40; // Altura do jogador
        ...
        this.animationFrames = {
            idle: Array(3).fill().map((_, i) => new Image()), // 4 frames para idle
            run: Array(12).fill().map((_, i) => new Image())  // 12 frames para run
        };
        
        // Carrega os sprites
        for (let i = 0; i < 3; i++) {
            this.animationFrames.idle[i].src = `assets/player_idle${i+1}.png`;
        }
        for (let i = 0; i < 12; i++) {
            this.animationFrames.run[i].src = `assets/player_run${i+1}.png`;
        }

        this.animationSpeeds = {
            idle: 0.4, // Mais lento que a run
            run: 0.08   // Mais rápido
        };
    ...
    updateAnimation(deltaTime) {
        ...
        if (Math.abs(this.velX) > 0 && this.grounded) {
            this.currentAnimation = 'run';
        } else {
            this.currentAnimation = 'idle';
        }
        
        // Atualiza o timer e frame da animação atual
        this.animationTimer += deltaTime;
        const currentSpeed = this.animationSpeeds[this.currentAnimation];
        ...
        // Garante que o frame atual seja válido
    ...
    draw() {
        ...
        if (this.currentAnimation === 'run') {
            frameToDraw = this.animationFrames.run[this.currentFrame];
        } else {
            frameToDraw = this.animationFrames.idle[this.currentFrame]; // Corrigido aqui
        }
        ...
        if (this.direction === 'right') {
            ...
            this.game.ctx.drawImage(
                frameToDraw, 
                -(this.x + this.width - this.game.cameraX), 
                this.y - this.game.cameraY, 
                this.width, 
                this.height
            );
...
class Game {
    ...
    gameLoop(timestamp) {
        ...
        const deltaTime = (timestamp - this.lastTime) / 1000; // Convertendo para segundos
        this.lastTime = timestamp;
        ...
        if (!this.menuActive && !this.paused) {
            this.player.update();
            this.player.updateAnimation(deltaTime); // Passa o deltaTime para a animação
        }
        this.draw();    
        requestAnimationFrame((t) => this.gameLoop(t)); // Passa o timestamp
    }
